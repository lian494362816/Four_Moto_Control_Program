...
unsigned char lock_BARO=0;//定高锁定当前以及清零用
...
//            if((flag.FlightMode==ULTRASONIC_High && lock==1 &&lock_spd_crl==0)|| (flag.FlightMode==ATMOSPHERE_High && lock_BARO==1 &&lock_spd_crl==0))
            keepheight_thr=thr_lpf;//这条语句是自己修改的,应该删除
//          exp_height=ultra_dis_lpf;//这条语句是自己修改的，应该删除
...
//            printf("thr_lpf=%f\r\n", thr_lpf);
            ...
            wz_acc_temp = my_sqrt(ABS(sensor.acc.averag.x*sensor.acc.averag.x) + ABS(sensor.acc.averag.y*sensor.acc.averag.y) + ABS(sensor.acc.averag.z*sensor.acc.averag.z)) - sensor.acc.quiet.z;
...
//            hc_speed_i = LIMIT( hc_speed_i, -500, 500 );//积分限幅	
            hc_speed_i = LIMIT( hc_speed_i, -1400, 1400 );//积分限幅	
//            printf("hc_speed_i=%f\r\n", hc_speed_i);
...
{
    exp_height_speed = ULTRA_SPEED *my_deathzoom_2(thr - 1350,80)/300.0f; //20这里具体根据自己起飞油门定，油门控制高度+-ULTRA_SPEEDmm / s
...
//    float temp1;
    ...
    userdata1[7]=wz_speed_pid_v.err = wz_speed_pid.kp *( exp_z_speed - wz_speed ) + 900;//这里用到了P,系数为0.25
//	printf("wz_speed_pid_v.err=%f\r\n", wz_speed_pid_v.err);//-946 ~ -1179
    userdata1[8]=wz_speed_pid_v.err_d = wz_speed_pid.kd * (-tempacc_lpf) *T + 450;//这里用的到了D
//    printf("wz_speed_pid_v.err_d=%f\r\n", wz_speed_pid_v.err_d);//  稳定-461
    wz_speed_pid_v.err_i += wz_speed_pid.ki *( exp_z_speed - h_speed ) *T;//期望速度与实际速度误差积分,这里用到了I
...
//    printf("wz_speed_pid_d.pid_out=%f\r\n", wz_speed_pid_v.pid_out);
//    printf("thr_keepheight=%f\r\n",thr_keepheight);//
//    temp1 =  wz_speed_pid_v.pid_out - thr_keepheight;
//    printf("temp1=%f\r\n", temp1);//值为0
//    printf("Thr_Weight=%f\r\n", Thr_Weight); //Thr_Weight = 1
    userdata1[10]=wz_speed_pid_v.pid_out-thr_keepheight;//这个值为-300
	printf("userdata1[10] =%f\r\n", userdata1[10]);
...
void Ultra_dataporcess(float T)
{
    ...
    if( ultra_dis_tmp < Ultrasonic_MAX_Height )//2.5m
    {	
